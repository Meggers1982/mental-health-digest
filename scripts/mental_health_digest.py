...
"""
    ...
    prompt = f"""You are a science writer and editor for a Mental Health & Psychiatry Research Digest, writing for a journalist audience covering mental health, psychiatry, psychology, behavioral science, and neuroscience as it relates to the mind and behavior.

Your readers pitch to publications like The Atlantic, STAT News, Psychology Today, HuffPost Health, Verywell Mind, The New York Times (Health/Well section), NPR Health, and science-forward outlets covering mental health and behavior.
...
- Generate MULTIPLE angles (2-3 max) only when the study genuinely fits different audiences with meaningfully different framings — e.g. a study on loneliness and brain structure could pitch differently to The Atlantic (cultural/societal angle) vs STAT News (clinical neuroscience angle) vs Psychology Today (accessible behavior science angle)
- Do not pad with extra angles if one covers it

Rules for content:
- Verify sample size (N), study design, population, and direction of effect against the abstract
- Distinguish between clinical populations (diagnosed disorder) and general community samples — findings do not always transfer between them
- For neurology in this context: prioritize studies with clear mental health relevance (depression and brain structure, anxiety and neuroinflammation, etc.) over pure movement disorder or injury research
- For behavioral sciences: note whether the study addresses a specific behavior, a broad construct, or a biological mechanism
- For psychology: distinguish between experimental lab studies and real-world observational or clinical research
- Flag WEIRD samples — findings from narrow Western student populations generalize poorly
...
- Be especially careful with neuroscience-to-behavior claims — brain imaging associations do not equal mechanisms
...
relevance_score rubric (1–10): start at 5, then adjust:
  +2 counterintuitive or overturns widely held belief about mental health or behavior
  +2 human subjects, community or clinical sample (N≥100)
  +1 clear public or personal relevance (everyday stress, relationships, sleep, social media)
  +1 clean design (RCT, longitudinal, large cohort, pre-registered)
  +1 addresses a high-prevalence condition (depression, anxiety, PTSD, loneliness, addiction)
  −1 per major caveat
  −1 WEIRD sample with limited generalizability
  −2 animal or cell study only
  −1 purely neuroimaging with no behavioral outcome
  ...
  Topic fit bonus: loneliness and social connection, social media and mental health, therapy effectiveness, psychedelic-assisted therapy, ADHD in adults, burnout, grief, sleep and mental health, trauma and PTSD, workplace mental health, youth mental health crisis score higher
{personalization}
...
{studies_block}"""
    ...
    try:
        ...
        return [{"pmid": s["pmid"], "headline": s["title"], "journal": s["journal"],
                 ...
                 "groundbreaking": "Major mental health or behavioral implication", "media_coverage": media_note,
...
def send_notification(category: str, chunk_label: str, study_count: int, run_date: str):
    subject = f"Mental Health & Psychiatry Research Digest — {run_date} | {study_count} {'Study' if study_count == 1 else 'Studies'}"
    html = f"""<!DOCTYPE html>
<html>
<body style="font-family:Georgia,serif;max-width:500px;margin:auto;padding:24px;color:#222;">
<h2 style="color:#1a3a2e;">Mental Health &amp; Psychiatry Research Digest</h2>
...
<p style="font-size:0.85em;color:#888;margin-top:2em;">
  Mental Health &amp; Psychiatry Research Digest · PubMed + Claude + SERPAPI
</p>
</body>
</html>"""
    try:
...
def main():
    ...
    print("=" * 60)
    print("Mental Health & Psychiatry Research Digest")
